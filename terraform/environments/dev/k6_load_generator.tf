...
data "aws_subnets" "k6_default_public" {
  count = var.k6_load_generator_enabled ? 1 : 0
  ...
  # Default subnets belong to the default VPC and have a route to its Internet Gateway.
  filter {
    name   = "default-for-az"
    values = ["true"]
  }
...
resource "aws_instance" "k6_load_generator" {
  ...
  subnet_id                   = sort(data.aws_subnets.k6_default_public[0].ids)[0]
